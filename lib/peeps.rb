...
# .all method returns result in reverse chronological order
  def self.all
    table = DatabaseConnection.query('SELECT * FROM peeps ORDER BY posttime DESC;')
  ...
  def self.filter(search)
    table = DatabaseConnection.query("SELECT * FROM peeps WHERE message ILIKE '%#{search}%' ORDER BY posttime DESC;")
    table.map { |result| Peep.new(result['id'], result['message'], result['posttime']) }
  end
end
