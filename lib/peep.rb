...
class Peep
  ...
  def self.all
    ...
    result = connection.exec("SELECT id, message, posted_date, TO_CHAR(posted_date, 'dd/mm/yyyy') 
    FROM peeps ORDER BY posted_date DESC;")
    result.map do |peep|
  ...
  def self.post(message:)
    ...
    result = connection.exec("INSERT INTO peeps (message) VALUES('#{message}')
     RETURNING id, message, posted_date;")
